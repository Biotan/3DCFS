...
parser.add_argument('--verbose', action='store_true', default=True,help='if specified, output color-coded seg obj files')
parser.add_argument('--log_dir', default='log5_test', help='Log dir [default: log]')
parser.add_argument('--num_point', type=int, default=4096, help='Point number [default: 4096]')
parser.add_argument('--bandwidth', type=float, default=0.6, help='Bandwidth for meanshift clustering [default: 1.]')
parser.add_argument('--input_list', type=str, default='meta/area5_data_label.txt', help='Input data list file [default: data/test_hdf5_file_list_Area5.txt]')
parser.add_argument('--model_path', type=str, default='log5/epoch_49.ckpt', help='Path of model [default: log/model.ckpt]')
    ...
    with GRAPH.as_default():
        ...
        for shape_idx in range(len_pts_files):
            ...
            for j in range(num_data):
                ...
                pts = cur_data[j,...] # cur_data: [90,4096,9]   pts: [4096 9]
                group = cur_group[j] # [4096,]
                sem = cur_sem[j] # [4096,]
                ...
                pred_val = np.squeeze(pred_ins_val, axis=0) # pred_val: [4096 5]
                pred_sem = np.squeeze(pred_sem_label_val, axis=0) # pred_sem: [4096,]
                pred_sem_softmax = np.squeeze(pred_sem_softmax_val, axis=0) # pred_sem_softmax: [4096,13]
                ...
                groupids = BlockMerging(volume, volume_seg, pts[:, 6:],
                                        groupids_block.astype(np.int32), group_seg, gap) # volume:[201 201 201] volume_seg: [201 201 201]
