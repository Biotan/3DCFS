...
for i in range(num_room):
    print(i)
    data_label = np.loadtxt(pred_data_label_filenames[i]) # [368640 9]
    pred_ins = data_label[:,-1].reshape(-1).astype(np.int) # [368640,] the last column
    pred_sem = data_label[:, -2].reshape(-1).astype(np.int) # [368640, ] the Second to last column
    ...
    for i_sem in range(NUM_CLASSES):
        ...
        for ig, ins_gt in enumerate(pts_in_gt[i_sem]):
            ...
            sum_cov += ovmax # Equ. 7
            mean_weighted_cov += ovmax * num_ins_gt_point # Equ. 8 not pure

        if len(pts_in_gt[i_sem]) != 0:
            mean_cov = sum_cov / len(pts_in_gt[i_sem]) # Equ. 7
            all_mean_cov[i_sem].append(mean_cov)

            mean_weighted_cov /= num_gt_point # Equ. 9 not pure
    ...
    for i_sem in range(NUM_CLASSES):
        ...
        for ip, ins_pred in enumerate(pts_in_pred[i_sem]):
            ...
            if ovmax >= at: # recall at 0.05, means the iou is 0.05
                    tp[ip] = 1  # true
            else:
